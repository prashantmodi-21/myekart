...
const Products = ({categories, products, addToCart, sortValue, selectValue, selectCategory}) => {
  ...
  return (
    ...
    <FormControl sx={{width: '160px', marginTop: '1rem'}}>
      ...
      <Select
        ...
        id="demo-simple-select"
        label="Category"
      >{categories.map((category)=>(<MenuItem key={category.id}><Checkbox onChange={()=>selectCategory(category.slug)}/><ListItemText primary={category.name}/></MenuItem>))}
      </Select>
    </FormControl></>}
    </Stack>
